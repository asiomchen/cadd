...
df = pd.read_csv('/home/anton/PycharmProjects/cadd/pocket_detection/pocket_random_subset.csv', sep=',')
...
with open('pocket_conjugates_2.smi', 'w') as f:
    f.writelines(conjugates)

ids = df['ID'].tolist()
ps_id = 1
link_id = 1
os.mkdir(f'../notebooks/sdf_pocket_set')
os.chdir(f'../notebooks/sdf_pocket_set')
for ids, conjugate in zip(ids, conjugates):
    name = f'{ps_id}_{link_id}_{ids}'
    print(conjugate)
    print(name)
    if conjugate != 'None':
        to_sdf(conjugate, name)
